...
void Update_first (List &L, item x); //Sua phan tu vao dau danh sach
void Insert_k (List &L, item x, int k); //Chen phan tu vao vi tri k trong danh sach
void Update_k (List &L, item x, int k); //Sua phan tu tai vi tri k trong danh sach
void Input (List &L);//Nhap danh sach
void Output (List L);//Xuat danh sach
int Search (List L, item x); //Tim phan tu x trong danh sach, ham tra ve vi tri cua phan tu tim duoc
...
void Update_first (List &L, item x)  //Sua x vao vi tri dau tien trong danh sach
{
    Node *P;
    P = Make_Node(P,x); //tao 1 Node P
    P->next = L; //Cho P tro den L
...
void Update_k(List &L, item x, int k) //sua x vao vi tri k trong danh sach
{
    Node *P, *Q = L;
    int i=1;
    if (k<1 || k> len(L)+1)
        printf("Vi tri sua khong hop le !"); //kiem tra dieu kien
    else
    {
        P = Make_Node(P,x); //tao 1 Node P
        if (k == 1)
            Update_first(L,x); //sua vao vi tri dau tien
        else //sua vao k != 1
        {
            while (Q != NULL && i != k-1) //duyet den vi tri k-1
            {
...
{
    while (Search(L,x))
        Del_k (L,x,Search(L,x)); //trong khi van tim thay x thi van xoa
}
...
void Input (List &L) //nhap danh sach
{
    int i=0;
    item x;
    printf("Moi ban nhap vao danh sach cac phan tu, nhap 0 de ket thuc: \n");
    do
    {
        i++;
...
int main()
{
    ...
    printf("\n5: Sua phan tu tai vi tri k trong DS");
    printf("\n6: Thoat");
    do
    {
        printf("\nBan chon: ");
        ...
        case 5:
        {
            item x;
            int k;
            printf ("Nhap phan tu can sua vao DS: ");
            scanf("%d",&x);
            printf ("Nhap vi tri can sua: ");
            scanf ("%d",&k);
            Update_k (L,x,k);
            printf ("DS sau khi sua:\n");
            Del_k (L,x,k+1);
            Output(L);
            break;
        }
